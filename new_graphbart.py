...
from utils.new_model_utils import GraphBARTMultiGPUWrapper
import torch
import math
...
# ])
InputData = namedtuple('InputData', [
    'paper_id', 'src_tokens', 'tgt_tokens', 'node_data', 'graph',
    'entity_num'
])
"""
paper_id
...
class GraphBART:
    ...
    def load_data(self, set_type, data):
        ...
        for datum in tqdm(data, total=len(data), desc=f'Loading {set_type} data...'):
            ...
            rel_tokens = [self._graphbart.encode(x,self._src_max_length) for x in rel_data[:]]
