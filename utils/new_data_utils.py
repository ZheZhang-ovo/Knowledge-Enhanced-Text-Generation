...
class Datum:
    ...
    """
    def __init__(self, paper_id,  oracle,review,
                 entities,  relations):
            ...
            for i in range(len(words)):
                ...
                if words[i]=="--":
                    if t1==t2:
                        ...
                        #取出loss function
                        if t2==t1+2:
                            relation_text = words[t1]+words[t1+1].upper()+words[t2]
                        else:
                            relation_text = words[t1]+words[t1+1].upper()+'-'+words[t1+2].upper()+words[t2]
                        #--USED-FOR--
                        tail = " ".join(words[t2+1:])
    ...
    def from_json(cls, json_data):
        ...
        #        json_data['relations'])
        return cls(json_data['id'], json_data['oracle'],  json_data['review'], json_data['entities'],
                   json_data['relations'])
